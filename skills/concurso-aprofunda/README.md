...
Versão atual: **0.12.0** (`flashcards_gen.py` deixa de apagar o baralho anterior: o `write_text` era incondicional e o plugin Spaced Repetition ancora o histórico de revisão no TEXTO DA FRENTE do cartão, então regerar zerava semanas de revisão sem apagar arquivo nenhum — agora pula o existente, reporta em `ja_existiam` e só regera com `--forcar`, com backup). Na 0.11.0: (a heurística que escolhe as leis do notebook deixa de casar número solto: `105` de "Cap. 3 — Ortografia 105–142" e "(p. 105)" mandava a LC do sigilo bancário para o notebook de **ortografia**, e `art. 109` puxaria a Resolução CNAS 109 — página é número, artigo é número, **norma é número com nome**; em troca, a norma citada só pelo apelido consagrado (**CDC**, LGPD, ECA, LOAS, PNAS) passa a casar, o que sozinho devolveu o CDC a `produtos-bancarios`, que o cita seis vezes sem escrever "8.078" nenhuma — sigla de **norma** entra, sigla de **órgão ou sistema** não, porque BACEN arrastava duas circulares e SUAS arrastava a NOB; e `migrar_fontes_notebook.py` declara `fontes_notebook:` no vault, com `--completar` para reexaminar o já declarado **acrescentando sem nunca remover**) (`validar_assuntos.py`: as seções do template deixam de ser opcionais — 25 assuntos do vault foram escritos com estrutura própria e perderam a seção `📝 Para estudar depois`, onde vivem 100% das tarefas, e cinco matérias inteiras apareceram no site sem tarefa nenhuma sem erro em lugar nenhum). Na 0.8.0: (ampliar um aprofundamento já escrito com uma fonte nova —
