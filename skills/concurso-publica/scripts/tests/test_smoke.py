...
# Os helpers de fixture moram em `fixture_concurso.py`: a suíte do `deploy.sh`
# usa os MESMOS, em vez de inventar um concurso próprio.
...
def test_manifesto_guarda_a_origem_no_vault():
    """Sem `origem`, o deploy não sabe reconstruir o que já está no build.

    O envio é `rsync --delete` do build INTEIRO, mas o `--concurso-dir` nomeia um
    concurso só. Foi assim que o BB_2027_PREVISTO foi republicado com um build de
    véspera enquanto se publicava o SEDES_2026 — sem erro nenhum. Este campo é o
    que o `deploy.sh` lê para reconstruir todos antes de enviar.
    """
    with tempfile.TemporaryDirectory() as d:
        base = _montar_concurso(Path(d) / "TESTE_2026")
        out = Path(d) / "site"
        _construir(base, out)
        man = json.loads((out / "teste_2026" / ".concurso.json")
                         .read_text(encoding="utf-8"))
        assert man["origem"] == str(base)
        assert Path(man["origem"]).is_dir()
